...
public class CMYKJPEGImageReader extends ImageReader {
	...
	private void readHeader() throws IOException {
		if (image == null) {
			...
			Object in = getInput();
			/*
			 * No need for JMF support in CMYKJPEGImageReader. if (in instanceof
			 * Buffer) { in = ((Buffer) in).getData(); }
			 */
	...
	public static BufferedImage read(ImageInputStream in, boolean inverseYCCKColors) throws IOException {
		...
		for (JFIFInputStream.Segment seg = fifi.getNextSegment(); seg != null; seg = fifi.getNextSegment()) {
			if (0xffc0 <= seg.marker && seg.marker <= 0xffc3 // 
				...
				if (seg.length >= 26) {
					DataInputStream dis = new DataInputStream(fifi);
					// Check for 12-bytes containing the null-terminated string:
					// "ICC_PROFILE".
				...
				if (seg.length == 12) {
					DataInputStream dis = new DataInputStream(fifi);
					// Check for 6-bytes containing the null-terminated string:
					// "Adobe".
	...
	private static ImageReader createNativeJPEGReader() {
		 ...
		 * ImageReader r = i.next(); if (!(r instanceof CMYKJPEGImageReader) &&
		 * !r.getClass().getName().contains("CMYKJPEGImageReader")) { return r;
		 * } }
		 * 
		 * return null;
		 */
	 ...
	 * @param cmykProfile
	 *            An ICC_Profile for conversion from the CMYK color space to the
	 ...
	 *            File Interchange Format (JFIF).
	 * @param rgbaProfile
	 *            An ICC_Profile for conversion from the RGBA color space to the
	...
	private static Raster convertInvertedYCCKToCMYK(Raster ycckRaster) {
		...
		Raster cmykRaster = Raster.createPackedRaster(new DataBufferInt(cmyk, cmyk.length), w, h, w, new int[] { 0xff000000, 0xff0000, 0xff00, 0xff }, null);
		return cmykRaster;
	...
	private static Raster convertYCCKtoCMYK(Raster ycckRaster) {
		...
		return Raster.createPackedRaster(new DataBufferInt(cmyk, cmyk.length), w, h, w, new int[] { 0xff000000, 0xff0000, 0xff00, 0xff }, null);
	}
